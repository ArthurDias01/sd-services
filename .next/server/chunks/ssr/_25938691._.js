module.exports = [
  ...
  (a) => {
    "use strict";
    a.s(["Toaster", () => c, "toast", () => d, "useSonner", () => e]);
    var b = a.i(11857);
  ...
  (a) => {
    ...
    async function o({ name: a, context: b, ...c }, d) {
      let e = l()?.tracer;
      if (!e) return d();
      let f = async (a) => {
        try {
    ...
    let r = Symbol.for("asyncDispose"),
      s = Symbol.asyncDispose ?? r;
    class t {
      #a = !1;
      #b = !1;
      #c;
    ...
    function u({ name: a, ...b }, c) {
      let d;
      return new t(
        async () => {
          d ??= m(a);
          try {
            let a = await p(d, () => c.next());
    ...
    let B = /^\s*function\s*\(\)\s*\{\s*\[native code\]\s*\}\s*$/,
      ...
      D = Symbol.for("__@orpc/client@1.13.5/error/ORPC_ERROR_CONSTRUCTORS__");
    ...
    class F extends Error {
      ...
      constructor(a, ...b) {
        ...
        if (void 0 !== c.status && !G(c.status))
          throw Error("[ORPCError] Invalid error status code.");
        (super(
          (function (a, b) {
            return b || C[a]?.message || a;
          })(a, c.message),
          c,
        ),
          (this.code = a),
          (this.status = (function (a, b) {
            return b ?? C[a]?.status ?? 500;
          })(a, c.status)),
          (this.defined = c.defined ?? !1),
          (this.data = c.data));
      }
      toJSON() {
        return {
      ...
      static [Symbol.hasInstance](a) {
        if (E.has(this)) {
          let b = z(a) ? Object.getPrototypeOf(a)?.constructor : null;
          if (b && E.has(b)) return !0;
        }
...
`);
        ...
        if (this.incomplete) throw new K("Event Iterator ended before complete");
      }
    }
    ...
    function O(a) {
      if (a.includes("\n")) throw new J("Event's id must not contain a newline character");
    }
    function P(a) {
      if (!Number.isInteger(a) || a < 0) throw new J("Event's retry must be a integer and >= 0");
    }
    function Q(a) {
      if (a.includes("\n")) throw new J("Event's comment must not contain a newline character");
    }
    function R(a) {
      let b = "";
      if (
...
`));
        ...
        if (a.event.includes("\n"))
          throw new J("Event's event must not contain a newline character");
        b += `event: ${a.event}
`;
      }
      return (
...
`;
    ...
    let S = Symbol("ORPC_EVENT_SOURCE_META");
    function T(a, b) {
      if (void 0 === b.id && void 0 === b.retry && !b.comments?.length) return a;
      if ((void 0 !== b.id && O(b.id), void 0 !== b.retry && P(b.retry), void 0 !== b.comments))
        for (let a of b.comments) Q(a);
      return new Proxy(a, { get: (a, c, d) => (c === S ? b : Reflect.get(a, c, d)) });
    }
    function U(a) {
      return z(a) ? Reflect.get(a, S) : void 0;
    }
    function V(a, b) {
      let c = async (a) => {
    ...
    class Y {
      constructor(a, b, c = {}) {
        ((this.codec = a),
          (this.sender = b),
          new X(c.plugins).init(c),
          (this.interceptors = i(c.interceptors)),
          (this.clientInterceptors = i(c.clientInterceptors)));
      }
      interceptors;
      clientInterceptors;
      call(a, b, c) {
        return o(
          { name: `${j}.${a.join("/")}`, signal: c.signal },
          (d) => (
            d?.setAttribute("rpc.system", j),
            d?.setAttribute("rpc.method", a.join(".")),
            q(b) && (b = u({ name: "consume_event_iterator_input", signal: c.signal }, b)),
            v(
              this.interceptors,
              { ...c, path: a, input: b },
              async ({ path: a, input: b, ...c }) => {
                let e,
                  f = l(),
                  g = f?.trace.getActiveSpan() ?? d;
                g && f && (e = f?.trace.setSpan(f.context.active(), g));
                let h = await o({ name: "encode_request", context: e }, () =>
                    this.codec.encode(a, b, c),
                  ),
                  i = await v(
                    this.clientInterceptors,
                    { ...c, input: b, path: a, request: h },
                    ({ input: a, path: b, request: c, ...d }) =>
                      o({ name: "send_request", signal: d.signal, context: e }, () =>
                        this.sender.call(c, d, b, a),
                      ),
                  ),
                  j = await o({ name: "decode_response", context: e }, () =>
                    this.codec.decode(i, c, a, b),
                  );
                return q(j) ? u({ name: "consume_event_iterator_output", signal: c.signal }, j) : j;
              },
            )
          ),
        );
    ...
    class $ {
      ...
      async encode(a, b, c) {
        var d;
        let e = "function" == typeof (d = await A(this.headers, c, a, b)).forEach ? W(d) : d;
        void 0 !== c.lastEventId &&
          (e = (function (a, b) {
            let c = { ...a };
            for (let a in b)
              Array.isArray(b[a])
                ...
                : void 0 !== b[a] &&
                  (Array.isArray(c[a])
                    ...
                    : void 0 !== c[a]
                      ...
                      : (c[a] = b[a]));
            return c;
          })(e, { "last-event-id": c.lastEventId }));
        let f = await A(this.expectedMethod, c, a, b),
          g = new URL(await A(this.baseUrl, c, a, b));
        g.pathname = `${g.pathname.replace(/\/$/, "")}/${a.map(encodeURIComponent).join("/")}`;
        let h = this.serializer.serialize(b);
        if ("GET" === f && !(h instanceof FormData) && !q(h)) {
          let d = await A(this.maxUrlLength, c, a, b),
            i = new URL(g);
          if ((i.searchParams.append("data", x(h)), i.toString().length <= d))
            return { body: void 0, method: f, headers: e, url: i, signal: c.signal };
        }
        return {
          url: g,
          method: "GET" === f ? this.fallbackMethod : f,
          headers: e,
          body: h,
          signal: c.signal,
        };
      ...
      async decode(a) {
        ...
        if (!b) {
          ...
          throw new F(
            ((d = a.status),
            Object.entries(C).find(([, a]) => a.status === d)?.[0] ??
    ...
    class _ {
      ...
      #e(a, b) {
        ...
        let h = new FormData();
        return (
          h.set("data", x({ json: c, meta: g, maps: e })),
          f.forEach((a, b) => {
            h.set(b.toString(), a);
          }),
          h
        ...
        return q(a)
          ? V(a, {
              ...
              error: async (a) => {
                ...
                let b = this.#f(a.data);
                return H(b) ? I(b, { cause: a }) : new L({ data: b, cause: a });
              },
            })
          : this.#f(a);
      }
    ...
    class ac {
      ...
      adapterInterceptors;
      constructor(a) {
        (new ab(a.plugins).initRuntimeAdapter(a),
          (this.fetch = a.fetch ?? globalThis.fetch.bind(globalThis)),
          (this.toFetchRequestOptions = a),
          (this.adapterInterceptors = i(a.adapterInterceptors)));
      }
      async call(a, b, c, d) {
        let e = (function (a, b = {}) {
              ...
              if ((b.delete("content-type"), b.delete("content-disposition"), void 0 !== a)) {
                ...
                return a instanceof FormData || a instanceof URLSearchParams
                  ...
                  : q(a)
                    ? (b.set("content-type", "text/event-stream"),
                      (function (a, b = {}) {
                        let c,
                          ...
                          h = b.eventIteratorInitialCommentEnabled ?? !0,
                          i = b.eventIteratorInitialComment ?? "",
                          j = !1;
                        return new ReadableStream({
                          start(a) {
                            ((d = m("stream_event_iterator")),
                              h && a.enqueue(R({ comments: [i] })));
                          },
                          async pull(b) {
                            try {
                              e &&
                                (c = setInterval(() => {
                                  (b.enqueue(R({ comments: [g] })), d?.addEvent("keepalive"));
                                }, f));
                              let h = await p(d, () => a.next());
                              if ((clearInterval(c), j)) return;
                              let i = U(h.value);
                              if (!h.done || void 0 !== h.value || void 0 !== i) {
                                let a = h.done ? "done" : "message";
                                (b.enqueue(R({ ...i, event: a, data: x(h.value) })),
                                  d?.addEvent(a));
                              }
                              ...
                              if ((clearInterval(c), j)) return;
                              (a instanceof L
                                ? (b.enqueue(R({ ...U(a), event: "error", data: x(a.data) })),
                                  d?.addEvent("error"),
                                  b.close())
                                : (n(d, a), b.error(a)),
                                d?.end());
                    ...
                    : (b.set("content-type", "application/json"), x(a));
              }
            })(a.body, c, b);
        ...
        return (function (a, b = {}) {
          ...
          return {
            body:
              ((c = () =>
                (function (a, b = {}) {
                  return o({ name: "parse_standard_body", signal: b.signal }, async () => {
                    let c = a.headers.get("content-disposition");
                    if ("string" == typeof c) {
                      let b =
                          (function (a) {
                            let b = a.match(/filename\*=(UTF-8'')?([^;]*)/i);
                            if (b && "string" == typeof b[2]) {
                              var c = b[2];
                              try {
                            ...
                            let d = a.match(/filename="((?:\\"|[^"])*)"/i);
                            if (d && "string" == typeof d[1]) return d[1].replace(/\\"/g, '"');
                          })(c) ?? "blob",
                        d = await a.blob();
                      return new File([d], b, { type: d.type });
                    }
                    let d = a.headers.get("content-type");
                    if (!d || d.startsWith("application/json")) return w(await a.text());
                    ...
                    if (d.startsWith("text/event-stream"))
                      return (function (a, b = {}) {
                        let c,
                          d = a?.pipeThrough(new TextDecoderStream()).pipeThrough(new N()),
                          e = d?.getReader(),
                          f = !1;
                        return new t(
                          async () => {
                            c ??= m("consume_event_iterator_stream");
                            try {
                              for (;;) {
                                if (void 0 === e) return { done: !0, value: void 0 };
                                let { done: a, value: b } = await p(c, () => e.read());
                                if (a) {
                                  if (f) throw new k("Stream was cancelled");
                                ...
                                switch (b.event) {
                                  ...
                                  case "error": {
                                    let a = new L({ data: w(b.data) });
                                    throw ((a = T(a, b)), c?.addEvent("error"), a);
                                  }
                                  case "done": {
                                    let a = w(b.data);
                                    return (
                              ...
                              throw (a instanceof L || n(c, a, b), a);
                            }
                          },
                          async (a) => {
                            try {
                              ("next" !== a && ((f = !0), c?.addEvent("cancelled")),
                                await p(c, () => e?.cancel()));
                            } catch (a) {
                              throw (n(c, a, b), a);
                            } finally {
                    ...
                    let e = await a.blob();
                    return new File([e], "blob", { type: e.type });
                  });
                })(a, b)),
              () => {
          ...
          await v(
            ...
            { ...b, request: e, path: c, input: d, init: { redirect: "manual" } },
    ...
    function ah(a, b) {
      ...
      if (g) {
        if (d) {
          if (b.queryHash !== aj(g, b.options)) return !1;
        } else if (!al(b.queryKey, g)) return !1;
      }
      if ("all" !== c) {
        let a = b.isActive();
    ...
    function ai(a, b) {
      ...
      if (f) {
        ...
        if (c) {
          if (ak(b.options.mutationKey) !== ak(f)) return !1;
        } else if (!al(b.options.mutationKey, f)) return !1;
      }
      return (!d || b.state.status === d) && (!e || !!e(b));
    }
    function aj(a, b) {
      return (b?.queryKeyHashFn || ak)(a);
    }
    function ak(a) {
    ...
    function al(a, b) {
      return (
        ...
        (typeof a == typeof b &&
          ...
          "object" == typeof a &&
          "object" == typeof b &&
          Object.keys(b).every((c) => al(a[c], b[c])))
      );
    }
    ...
    let aw = Symbol("ORPC_OPERATION_CONTEXT");
    var ax =
        ((b = []),
        (c = 0),
        (d = (a) => {
    ...
    function aC(a) {
      return (a ?? "online") !== "online" || aA.isOnline();
    }
    ...
    function aE(a) {
      let b,
        ...
        f = () => az.isFocused() && ("always" === a.networkMode || aA.isOnline()) && a.canRun(),
        g = () => aC(a.networkMode) && a.canRun(),
        h = (a) => {
    ...
    var aF = class {
        ...
        clearGcTimeout() {
          this.#l && (ae.clearTimeout(this.#l), (this.#l = void 0));
        }
      },
      aG = class extends aF {
        #m;
        ...
        setOptions(a) {
          if (
            ...
            let a = aI(this.options);
            void 0 !== a.data && (this.setState(aH(a.data, a.dataUpdatedAt)), (this.#m = a));
          }
        }
        optionalRemove() {
          ...
          let e =
            ...
            "function" == typeof (d = this.options).structuralSharing
              ...
              : !1 !== d.structuralSharing
                ...
                : a);
          return (
            this.#t({ data: e, type: "success", dataUpdatedAt: b?.updatedAt, manual: b?.manual }), e
          );
        }
        setState(a, b) {
        ...
        cancel(a) {
          let b = this.#q?.promise;
          return (this.#q?.cancel(a), b ? b.then(af).catch(af) : Promise.resolve());
        }
        destroy() {
        ...
        isDisabled() {
          return this.getObserversCount() > 0
            ? !this.isActive()
            : this.options.queryFn === as ||
                this.state.dataUpdateCount + this.state.errorUpdateCount === 0;
        }
        isStatic() {
          return (
            this.getObserversCount() > 0 &&
            this.observers.some((a) => "static" === ag(a.options.staleTime, this))
          );
        }
        isStale() {
          return this.getObserversCount() > 0
        ...
        async fetch(a, b) {
          ...
          let d = new AbortController(),
            e = (a) => {
              Object.defineProperty(a, "signal", {
                enumerable: !0,
                get: () => ((this.#s = !0), d.signal),
              });
            },
            f = () => {
              let a,
                c = at(this.options, b),
                d = (e((a = { client: this.#p, queryKey: this.queryKey, meta: this.meta })), a);
          ...
          (this.options.behavior?.onFetch(g, this),
            ...
            ("idle" === this.state.fetchStatus || this.state.fetchMeta !== g.fetchOptions?.meta) &&
              this.#t({ type: "fetch", meta: g.fetchOptions?.meta }),
            (this.#q = aE({
              initialPromise: b?.initialPromise,
              fn: g.fetchFn,
              onCancel: (a) => {
                (a instanceof aD && a.revert && this.setState({ ...this.#n, fetchStatus: "idle" }),
                  d.abort());
              },
              onFail: (a, b) => {
          ...
          let b = (b) => {
            switch (a.type) {
              ...
              case "success":
                ...
                return ((this.#n = a.manual ? d : void 0), d);
              case "error":
                let e = a.error;
                return {
                  ...b,
    ...
    var aJ = class extends ay {
        ...
        find(a) {
          let b = { exact: !0, ...a };
          return this.getAll().find((a) => ah(b, a));
        }
        findAll(a = {}) {
          let b = this.getAll();
          return Object.keys(a).length > 0 ? b.filter((b) => ah(a, b)) : b;
        }
        notify(a) {
          ax.batch(() => {
            this.listeners.forEach((b) => {
        ...
        async execute(a) {
          ...
          try {
            if (d) b();
              ...
              (this.#t({ type: "pending", variables: a, isPaused: e }),
                this.#w.config.onMutate && (await this.#w.config.onMutate(a, this, c)));
              let b = await this.options.onMutate?.(a, c);
              b !== this.state.context &&
                this.#t({ type: "pending", context: b, variables: a, isPaused: e });
            }
            let f = await this.#q.start();
            return (
              await this.#w.config.onSuccess?.(f, a, this.state.context, this, c),
      ...
      aL = class extends ay {
        ...
        find(a) {
          let b = { exact: !0, ...a };
          return this.getAll().find((a) => ai(b, a));
        }
        findAll(a = {}) {
          return this.getAll().filter((b) => ai(a, b));
        }
        notify(a) {
          ax.batch(() => {
            this.listeners.forEach((b) => {
        ...
        resumePausedMutations() {
          ...
          return ax.batch(() => Promise.all(a.map((a) => a.continue().catch(af))));
        }
      };
    function aM(a) {
              ...
              let c = !1,
                ...
                k = async (a, d, e) => {
                  ...
                  let f = (() => {
                      ...
                      return (
                        ...
                        Object.defineProperty(i, "signal", {
                          ...
                          get: () => (
                            ...
                            h ||
                              ((h = !0),
                              g.aborted ? f() : g.addEventListener("abort", f, { once: !0 })),
                            g
                          ),
                        }),
                        i
                    ...
                    i = e ? ar : aq;
                  return { pages: i(a.pages, g, h), pageParams: i(a.pageParams, d, h) };
                };
              if (e && f.length) {
                let a = "backward" === e,
                ...
                let b = a ?? f.length;
                do {
                  let a = 0 === i ? (g[0] ?? d.initialPageParam) : aO(d, h);
                  if (i > 0 && null == a) break;
                  ((h = await k(h, a)), i++);
                } while (i < b);
              }
    ...
    function aO(a, { pages: b, pageParams: c }) {
      let d = b.length - 1;
    ...
    var aP = class {
        ...
        #F;
        constructor(a = {}) {
          ((this.#A = a.queryCache || new aJ()),
            (this.#w = a.mutationCache || new aL()),
            (this.#r = a.defaultOptions || {}),
            (this.#B = new Map()),
            (this.#C = new Map()),
            (this.#D = 0));
        ...
        mount() {
          (this.#D++,
            1 === this.#D &&
              ((this.#E = az.subscribe(async (a) => {
                a && (await this.resumePausedMutations(), this.#A.onFocus());
              })),
              (this.#F = aA.subscribe(async (a) => {
        ...
        ensureQueryData(a) {
          ...
          return void 0 === d
            ? this.fetchQuery(a)
            : (a.revalidateIfStale && c.isStaleByTime(ag(b.staleTime, c)) && this.prefetchQuery(b),
              Promise.resolve(d));
        }
        getQueriesData(a) {
        ...
        setQueryData(a, b, c) {
          ...
          if (void 0 !== g) return this.#A.build(this, d).setData(g, { ...c, manual: !0 });
        }
        ...
        cancelQueries(a, b = {}) {
          let c = { revert: !0, ...b };
          return Promise.all(ax.batch(() => this.#A.findAll(a).map((a) => a.cancel(c))))
            .then(af)
            .catch(af);
        }
        ...
        fetchQuery(a) {
          ...
          return c.isStaleByTime(ag(b.staleTime, c)) ? c.fetch(b) : Promise.resolve(c.state.data);
        }
        prefetchQuery(a) {
          return this.fetchQuery(a).then(af).catch(af);
        }
        fetchInfiniteQuery(a) {
          return ((a.behavior = aN(a.pages)), this.fetchQuery(a));
        }
        prefetchInfiniteQuery(a) {
          return this.fetchInfiniteQuery(a).then(af).catch(af);
        }
        ensureInfiniteQueryData(a) {
          return ((a.behavior = aN(a.pages)), this.ensureQueryData(a));
        }
        resumePausedMutations() {
          return aA.isOnline() ? this.#w.resumePausedMutations() : Promise.resolve();
        }
        getQueryCache() {
          return this.#A;
        }
        ...
        setQueryDefaults(a, b) {
          this.#B.set(ak(a), { queryKey: a, defaultOptions: b });
        }
        getQueryDefaults(a) {
          let b = [...this.#B.values()],
            c = {};
          return (
            b.forEach((b) => {
              al(a, b.queryKey) && Object.assign(c, b.defaultOptions);
            }),
            c
          );
        }
        setMutationDefaults(a, b) {
          this.#C.set(ak(a), { mutationKey: a, defaultOptions: b });
        }
        getMutationDefaults(a) {
          ...
          return (
            b.forEach((b) => {
              al(a, b.mutationKey) && Object.assign(c, b.defaultOptions);
            }),
            c
          );
        }
        defaultQueryOptions(a) {
          ...
          return (
            b.queryHash || (b.queryHash = aj(b.queryKey, b)),
    ...
    new aP({
      queryCache: new aJ({
        onError: (a, b) => {
          aQ.toast.error(`Error: ${a.message}`, {
            action: {
              label: "retry",
              onClick: () => {
    ...
    let aR = (function a(b, c = {}) {
      let d = c.path ?? [];
      return (function a(b) {
        return new Proxy(b, {
          get(b, c, d) {
            let e = Reflect.get(b, c, d);
            return "then" !== c || "function" != typeof e
              ? e
              : new Proxy(e, {
                  apply(c, d, e) {
                    if (
    ...
    (!(function a(b, c = {}) {
      ...
      let e,
        ...
        g = { key: (a) => au(f, a) },
        h =
          ((d = { path: f, experimental_defaults: c.experimental_defaults }),
          (e = {
            call: b,
            queryKey: (...[a = {}]) =>
              (a = { ...d.experimental_defaults?.queryKey, ...a }).queryKey ??
              au(d.path, { type: "query", input: a.input }),
            queryOptions(...[a = {}]) {
              a = { ...d.experimental_defaults?.queryOptions, ...a };
              let c = e.queryKey(a);
              return {
                queryFn: ({ signal: d }) => {
                  if (a.input === as)
                  ...
                  return b(a.input, {
                    signal: d,
                    context: { [aw]: { key: c, type: "query" }, ...a.context },
                  });
                },
                ...(a.input === as ? { enabled: !1 } : {}),
                ...a,
                queryKey: c,
              };
            },
            experimental_streamedKey: (...[a = {}]) =>
              (a = { ...d.experimental_defaults?.experimental_streamedKey, ...a }).queryKey ??
              au(d.path, { type: "streamed", input: a.input, fnOptions: a.queryFnOptions }),
            experimental_streamedOptions(...[a = {}]) {
              a = { ...d.experimental_defaults?.experimental_streamedOptions, ...a };
              let c = e.experimental_streamedKey(a);
              return {
                queryFn: (function (a, { refetchMode: b = "reset", maxChunks: c = 1 / 0 } = {}) {
                  return async (d) => {
                    let e = d.client.getQueryCache().find({ queryKey: d.queryKey, exact: !0 }),
                      f = !!e && void 0 !== e.state.data;
                    f &&
                      ("reset" === b
                        ? e.setState({
                            status: "pending",
                        ...
                        : d.client.setQueryData(d.queryKey, (a = []) => av(a, c)));
                    let g = [],
                      h = await a(d),
                      i = !f || "replace" !== b;
                    for await (let a of (d.client.setQueryData(d.queryKey, (a = []) => av(a, c)),
                    h)) {
                      if (d.signal.aborted) throw d.signal.reason;
                      (g.push(a),
                        (g = av(g, c)),
                        i && d.client.setQueryData(d.queryKey, (b = []) => av([...b, a], c)));
                    }
                    i || d.client.setQueryData(d.queryKey, g);
                    let j = d.client.getQueryData(d.queryKey);
                    return j ? av(j, c) : g;
                  };
                })(async ({ signal: d }) => {
                  if (a.input === as)
                    throw Error("queryFn should not be called with skipToken used as input");
                  let e = await b(a.input, {
                    signal: d,
                    context: { [aw]: { key: c, type: "streamed" }, ...a.context },
                  });
                  if (!q(e)) throw Error("streamedQuery requires an event iterator output");
                  return e;
                }, a.queryFnOptions),
                ...(a.input === as ? { enabled: !1 } : {}),
                ...a,
            ...
            experimental_liveKey: (...[a = {}]) =>
              (a = { ...d.experimental_defaults?.experimental_liveKey, ...a }).queryKey ??
              au(d.path, { type: "live", input: a.input }),
            experimental_liveOptions(...[a = {}]) {
              var c;
              a = { ...d.experimental_defaults?.experimental_liveOptions, ...a };
              let f = e.experimental_liveKey(a);
              return {
                queryFn:
                  ((c = async ({ signal: c }) => {
                    if (a.input === as)
                      throw Error("queryFn should not be called with skipToken used as input");
                    let d = await b(a.input, {
                      signal: c,
                      context: { [aw]: { key: f, type: "live" }, ...a.context },
                    });
                    if (!q(d)) throw Error("liveQuery requires an event iterator output");
                    return d;
                  }),
                  async (a) => {
                    let b;
                    for await (let d of await c(a)) {
                      if (a.signal.aborted) throw a.signal.reason;
                      ((b = { chunk: d }), a.client.setQueryData(a.queryKey, d));
                    }
                    if (!b)
                      throw Error(
                        `Live query for ${x(a.queryKey)} did not yield any data. Ensure the query function returns an AsyncIterable with at least one chunk.`,
                      );
                    return b.chunk;
                  }),
                ...(a.input === as ? { enabled: !1 } : {}),
            ...
            infiniteKey: (a) =>
              (a = { ...d.experimental_defaults?.infiniteKey, ...a }).queryKey ??
              au(d.path, {
                type: "infinite",
                input: a.input === as ? as : a.input(a.initialPageParam),
              }),
            infiniteOptions(a) {
              a = { ...d.experimental_defaults?.infiniteOptions, ...a };
              let c = e.infiniteKey(a);
              return {
                queryFn: ({ pageParam: d, signal: e }) => {
                  ...
                  return b(a.input(d), {
                    signal: e,
                    context: { [aw]: { key: c, type: "infinite" }, ...a.context },
                  });
                },
                ...(a.input === as ? { enabled: !1 } : {}),
                ...a,
            ...
            mutationKey: (...[a = {}]) =>
              (a = { ...d.experimental_defaults?.mutationKey, ...a }).mutationKey ??
              au(d.path, { type: "mutation" }),
            mutationOptions(...[a = {}]) {
              a = { ...d.experimental_defaults?.mutationOptions, ...a };
              let c = e.mutationKey(a);
              return {
                mutationFn: (d) =>
                  b(d, { context: { [aw]: { key: c, type: "mutation" }, ...a.context } }),
                ...a,
                mutationKey: c,
              };
            },
      ...
      return new Proxy(
        ...
        {
          get(d, e) {
            let g = Reflect.get(d, e);
            if ("string" != typeof e) return g;
            let h = a(b[e], {
              ...c,
              path: [...f, e],
              experimental_defaults: (function (a, b) {
                let c = a;
                for (let a of b) {
                  if (!z(c)) return;
                  c = c[a];
                ...
                return c;
              })(c.experimental_defaults, [e]),
            });
            return "function" != typeof g ? h : new Proxy(g, { get: (a, b) => Reflect.get(h, b) });
          },
        },
      );
    })(aR),
      a.s(["client", 0, aR], 6460));
  },
];

//# sourceMappingURL=_25938691._.js.map
